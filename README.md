...
`claude`를 실행하면 Claude가 `CLAUDE.md`를 자동으로 읽고 내 AWS 환경 전체를 파악합니다. 그냥 하고 싶은 것을 자연어로 말하면 됩니다.

```
"EC2 인스턴스 현황 파악해줘"
"이번 달 비용이 얼마나 나올 것 같아?"
"사용하지 않는 리소스 찾아줘"
"dev-service 지우고 싶은데 뭘 삭제해야 해?"
```

슬래시 커맨드는 자주 쓰는 작업을 빠르게 실행하는 단축키일 뿐입니다.
